#!/usr/bin/env python
# sp800_22_serial_test.py
#
# Copyright (C) 2017 David Johnston
# This program is distributed under the terms of the GNU General Public License.
#
# This file is part of sp800_22_tests.
#
# sp800_22_tests is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# sp800_22_tests is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with sp800_22_tests.  If not, see <http://www.gnu.org/licenses/>.

from __future__ import print_function

import math
# CORRECTED LINE: Replaced the custom 'gamma_functions' with the standard SciPy library
from scipy.special import gammaincc


def int2patt(n, m):
    """Convert integer to bit pattern of length m."""
    return [(n >> i) & 1 for i in range(m)]


def countpattern(patt, bits, n):
    """Count how many times patt appears in bits[:n]."""
    thecount = 0
    for i in range(n):
        match = True
        for j in range(len(patt)):
            if patt[j] != bits[i + j]:
                match = False
                break
        if match:
            thecount += 1
    return thecount


def psi_sq_mv1(m, n, padded_bits):
    """Compute psi-squared value for given m."""
    if n <= 0:
        raise ValueError("psi_sq_mv1: n=0 (invalid input length).")
    if m < 1:
        raise ValueError(f"psi_sq_mv1: m={m} must be >= 1.")

    counts = []
    for i in range(2**m):
        pattern = int2patt(i, m)
        count = countpattern(pattern, padded_bits, n)
        counts.append(count)

    psi_sq_m = sum(count**2 for count in counts)
    psi_sq_m = psi_sq_m * (2**m) / n
    psi_sq_m -= n
    return psi_sq_m


def serial_test(bits, patternlen=None):
    """Serial test implementation with safety checks."""
    n = len(bits)

    # --- safety check: no bits
    if n <= 0:
        raise ValueError("Error: Input sequence is empty. Cannot perform test.")

    # --- choose m
    if patternlen is not None:
        m = patternlen
    else:
        # estimate m from n
        m = int(math.floor(math.log(n, 2))) - 2
        if m < 4:
            raise ValueError(
                f"Error: Not enough data (n={n}) to use minimum m=4."
            )
        m = 4  # force minimum m = 4

    # --- ensure m is large enough for m-1, m-2
    if m < 3:
        raise ValueError(f"Error: m={m} is too small for serial test.")

    # Step 1: pad bits
    padded_bits = bits + bits[0:m - 1]

    # Step 2: compute psi values
    psi_sq_m = psi_sq_mv1(m, n, padded_bits)
    psi_sq_mm1 = psi_sq_mv1(m - 1, n, padded_bits)
    psi_sq_mm2 = psi_sq_mv1(m - 2, n, padded_bits)

    delta1 = psi_sq_m - psi_sq_mm1
    delta2 = psi_sq_m - (2 * psi_sq_mm1) + psi_sq_mm2

    # Step 3: compute probabilities
    P1 = gammaincc(2**(m - 2), delta1 / 2.0)
    P2 = gammaincc(2**(m - 3), delta2 / 2.0)

    success = (P1 >= 0.01) and (P2 >= 0.01)
    return (success, None, [P1, P2])


if __name__ == "__main__":
    bits = [0, 0, 1, 1, 0, 1, 1, 1, 0, 1]
    # This will now run without error.
    success, _, plist = serial_test(bits, patternlen=3)

    print("success =", success)
    print("plist = ", plist)